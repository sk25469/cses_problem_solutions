...
vector<ll> dijkstra(int numNodes, int start)
{
    bool visited[numNodes] = {false};
    vector<ll> dist(numNodes, INT64_MAX);
    // to show the min path we store the parent values in array
    vector<int> prev(numNodes, -1);
    dist[start] = 0;
    priority_queue<pll, vector<pll>, myComp> q;
    q.push({start, 0});
    while (!q.empty())
    {
        ...
        for (auto edge : adj[index])
        {
            ...
            ll newDist = dist[index] + edge.second;
