...
ll bellmanFord(int src, int end)
{
    vll dist(n + 1);
    ...
    for (int i = 1; i <= n - 1; i++)
    {
        for (int j = 0; j < m; j++)
        {
            int frm = edges[j].first;
            int to = edges[j].second.first;
            ll weight = edges[j].second.second;

            if (dist[frm] == INF)
                continue;

            dist[to] = min(dist[to], weight + dist[frm]);
    ...
    for (int j = 1; j < n; j++)
    {
        for (int i = 0; i < m; i++)
        {
            int frm = edges[i].first;
            int to = edges[i].second.first;
            ll weight = edges[i].second.second;
