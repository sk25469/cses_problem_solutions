...
void solve()
{
    ...
    vll dist(n + 1, INT64_MAX);
    vi path(n + 1, 0), minPath(n + 1, 0), maxPath(n + 1, 0);
    bool vis[n + 1] = {false};
    ...
    while (!pq.empty())
    {
        ...
        {
            int node = next.first;
            ll tempCost = next.second + distCurr;

            if (tempCost == dist[node])
            {
                path[node] = (path[node] + path[curr]) % MOD;
                minPath[node] = min(minPath[node], minPath[curr] + 1);
                maxPath[node] = max(maxPath[node], maxPath[curr] + 1);
            }
            else if (tempCost < dist[node])
            {
                path[node] = path[curr];
                minPath[node] = minPath[curr] + 1;
                maxPath[node] = maxPath[curr] + 1;
                dist[node] = tempCost;
    ...
    cout << dist[n] << " " << path[n] << " " << minPath[n] << " " << maxPath[n] << endl;
}
